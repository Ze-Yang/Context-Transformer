...
class Activation(nn.Module):
    def __init__(self, in_planes, feature_dim, out_planes):
        super(Activation, self).__init__()
        self.in_planes = in_planes
        self.feature_dim = feature_dim
        self.out_planes = out_planes

        self.Conv = nn.Conv2d(self.in_planes, self.feature_dim, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.fc = nn.Conv2d(self.feature_dim, self.out_planes, kernel_size=1)
    def forward(self, x):
        x = self.Conv(x)
        x = self.relu(x)
        x = self.fc(x)
...
def multibox(size, vgg, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        if k == 0:
            loc_layers += [Activation(512, feature_dim, cfg[k] * 4)]
            conf_layers += [Activation(512, feature_dim, cfg[k] * num_classes)]
            obj_layers += [Activation(512, feature_dim, cfg[k] * 2)]
        else:
    ...
    for k, v in enumerate(extra_layers):
        if k < indicator or k%2 == 0:
            loc_layers += [Activation(v.out_channels, feature_dim, cfg[i] * 4)]
            conf_layers += [Activation(v.out_channels, feature_dim, cfg[i] * num_classes)]
            obj_layers += [Activation(v.out_channels, feature_dim, cfg[i] * 2)]
            i += 1
