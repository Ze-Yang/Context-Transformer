...
def test_net(save_folder, net, detector, cuda, test_support, test_query, transform, max_per_image=300, thresh=0.005):
    ...
    for _ in range(args.support_episodes):
        ...
        s_conf_data_list = [s_conf_data[s_conf_t == i].view(-1, feature_dim) for i in range(1, num_classes)]
        tmp_list = [torch.stack([net.nonlinear(item[j]) for j in range(item.size(0))], 0) for item in s_conf_data_list]
        way_list = [torch.cat((way_list[i], tmp_list[i]), 0) for i in range(n_way)]
    way_list = [(item / torch.norm(item, dim=1, keepdim=True)).mean(0) for item in way_list]
    s_conf_cls_mean = torch.stack([item / torch.norm(item) for item in way_list], 0)  # [n_way, num_classes]
    ...
    for i in range(num_images):
        ...
        q_conf = ((q_conf_data.squeeze().mm(s_conf_cls_mean.t())) * net.scale).unsqueeze(0) # [1, num_priors, n_way]
