...
def test_net(save_folder, net, detector, cuda, test_support, test_query, transform, max_per_image=300, thresh=0.005):
    ...
    sampler = EpisodicBatchSampler(n_classes=len(test_support), n_way=len(test_support), n_episodes=args.support_episodes, phase='test')
    batch_iterator = iter(data.DataLoader(test_support, batch_sampler=sampler, num_workers=args.num_workers,
    ...
    if args.cuda:
        s_conf_cls_mean_sum = torch.zeros(n_way, feature_dim).cuda()
    else:
        s_conf_cls_mean_sum = torch.zeros(n_way, feature_dim)

    for i in range(args.support_episodes):
        # load support data
        # for i in range(10):
        ...
        #     a = s_conf_data[s_conf_t == i]
        #     b = a.view(-1, feature_dim).mean(0)
        # a = [s_conf_data[s_conf_t == i].view(-1, feature_dim).mean(0) for i in range(1, num_classes)]
        # for i in range(20):
        #     if torch.isnan(a[i]).any():
        #         a = 0
        s_conf_cls_mean = torch.stack([s_conf_data[s_conf_t == i].view(-1, feature_dim).mean(0) for i in range(1, num_classes)], 0)
        # s_pos = s_obj_t.view(n_way, n_shot, -1).unsqueeze(3).expand_as(s_conf_data)
                                                                            # [n_way, n_shot, num_priors, feature_dim]
        # s_conf_cls_mean = torch.stack([s_conf_data[i][s_pos[i]].view(-1, feature_dim).mean(0) for i in range(n_way)],
        #                               0)  # [n_way, feature_dim]
        s_conf_cls_mean_sum += s_conf_cls_mean
    s_conf_cls_mean_avg = s_conf_cls_mean_sum / args.support_episodes
    s_conf_cls_mean = s_conf_cls_mean_avg.unsqueeze(0).unsqueeze(0).expand(1, num_priors, n_way, feature_dim)
    ...
    for i in range(num_images):
        ...
        q_conf_data = q_conf_data.unsqueeze(2).expand(1, num_priors, n_way, feature_dim)
        q_conf = -torch.norm(q_conf_data - s_conf_cls_mean, dim=-1,
                             keepdim=False)  # [1, num_priors, n_way]
